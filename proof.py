...
for line in lines:
	...
	if "server_ip=" in line:
		server_ip_conf = line.split('=')[1]
	if "threads=" in line:
		threads_conf = line.strip('threads=')
	if "tor=" in line:
	...
	if "name=" in line:
		mname = line.split('=')[1]
	if "unitcount=" in line:
		unitcount = int(line.split('=')[1])
...
def getarr():
	...
	do_arr = [('%0x' % getrandbits(32)) for i in range(nonce_time*unitcount)]
	return do_arr
				...
				for nonce in possibles:
					...
					if xdiffx < diff:
						...
						try:
							...
							s1.connect((server_ip_conf, int(port)))  # connect to pool
...
def runit():
	...
	while True:
		try:
			...
			s.connect((server_ip_conf, int(port)))  # connect to pool
			...
			instances = range(int(threads_conf))
			thr = int(threads_conf)
	
			for q in instances:
				p = Process(target=miner, args=(str(q + 1), paddress, db_block_hash, diff, mining_condition, mining_condition_bin, netdiff, hq, thr, dh))
				p.daemon = True
				p.start()
			print("{} solutions at tasks {} and {}".format(threads_conf,str(diff),str(mining_condition)))
			...
			print(e)
			print("Unable to connect to pool check your connection or IP settings.")
			time.sleep(1)
	# verify connection
